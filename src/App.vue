      ...
      <div class="wnrl_k_zuo">
        <div class="wnrl_xuanze_top">
          ...
          <input
            ...
            value="回到今天"
          />
          <span class="wnrl_xuanze_top_wenzi">{{currentYear}}年{{currentMonth}}月</span>
        </div>
        ...
        <div
          class="wnrl_kongbai"
          v-for="item in firstBlank"
          :key="item"
        ></div>
        <div class="wnrl_riqi" v-for="item in dateArr" :key="item.glText">
          <a href="javascript:;" :class="[item.aClass, {'active': item.glText === currentDay}]" @click="clickDay(item.glText)"
        ...
        <div
          class="wnrl_kongbai"
          v-for="item in lastBlank"
          :key="item"
        ></div>
      </div>
      <div class="wnrl_k_you" v-for="item in detailArr"
          :key="item.nongli" :style="{'display': item.riqi === currentDay ? 'block': 'none'}">
  ...
  methods: {
    fetchDetail() {
      ...
      fetch(`/api/ajax/?q=${query}`, {
        ...
        .then((response) => {
          const { firstBlank, lastBlank, dateArr, detailArr } = getDocData(response);
